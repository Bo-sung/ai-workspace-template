...
Commit messages must follow `.agents/rules/commit_policy.md`. Project policy overrides any tool default that auto-appends `Co-Authored-By`, model/tool signatures, `Generated by` lines, or robot emoji. The tracked `commit-msg` hook at `.agents/git-hooks/commit-msg` enforces this — never bypass with `--no-verify`.
