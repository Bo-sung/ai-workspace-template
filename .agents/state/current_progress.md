...
Last Updated: 2026-05-31T17:42:11+09:00

## Completed

- **CLIENT_APP BattleSim.Core v0.8 Support Upgrade 클라이언트 미러 및 uGUI HUD 통합 완료 (2026-06-02)**: BattleSim.Core v0.8 Support Upgrade(Resource, Pilot 트랙 각 최대 5레벨) 피처를 Unity 클라이언트에 미러링하고 HUD 및 시각 피드백 연출을 오류 0개 수준으로 통합 완료하였습니다. `StageBattleSession`에 local player (SideA) 업그레이드 Submit API와 simulator fault 방지를 위한 Dynamic Energy 비용 및 파일럿 배치 제한 사전검증을 구현하였습니다. `BattlePresenter` 및 `HudViewModel`에 support 상태 필드 및 F1/F2 단축키 매핑을 바인딩하고, `UI_BattleHud`에서 실시간 UI 렌더링 및 onClick 이벤트를 처리하였습니다. UIHUD 프리팹에 Support Upgrade UI 요소를 멱등하게 생성/배치하고 직렬화 필드를 연결하는 에디터 툴 `AddSupportUpgradeUIToUIHUD`을 구현하였으며, `BattleWorldView.ProcessEvent`에서 `SupportUpgradeStarted` 및 `SupportUpgradeCompleted` 이벤트를 수신하여 양 진영 베이스 상단에 플로팅 텍스트 피드백을 노출하도록 처리하여 conventionally commit 4건으로 분할 기록하였습니다.
- **SHARED_BATTLE_CORE BattleSim.Core v0.8 Support Upgrade 구현 및 검증 강화 (2026-06-01)**: 전투 중 임시적으로 활성화되는 Support Upgrade 시스템(Resource, Pilot 트랙 각 최대 5레벨)을 빌드 경고 0개, 오류 0개 수준으로 완벽하게 구현하였습니다. `BattleSupportTrack` enum 및 `BattleSideSupportState` snapshot 클래스를 신규 정의하고, `BattleCommand.StartSupportUpgrade` 명령 팩토리와 `SupportUpgradeStarted/Completed` 결정론적 이벤트 방출 모델을 통합하였습니다. 시뮬레이션 AdvancedTick 내에서 업그레이드 활성화 시 에너지 regen 중단 및 progress tick 처리, 완료 후 절대 max energy 계산 및 regen 보너스 적용, 파일럿 스탯 배수 보너스 실시간 적용을 fixed-point 및 determinism 정책에 부합하도록 구현 완료하였습니다. 특히 `StartSupportUpgrade` 팩토리 및 시뮬레이터 내부 `ApplyStartSupportUpgrade` 단계에 Resource/Pilot 트랙 외 잘못된 enum이나 정의되지 않은 `(BattleSupportTrack)999` 유입을 엄격히 방지하는 이중 예외 검증 레이어를 도입하였습니다. 리플렉션 인젝션을 포함한 16종의 신규 v0.8 단위 테스트를 `SimulatorTests.cs` 에 추가 및 실행하여 기존 레그레션 테스트 회귀 없이 100% 검증 통과를 실증하였습니다.
- **CLIENT_APP StageBattleWorldView redesign (2026-05-31)**: Redesigned the visual rendering system of `StageBattleWorldView` into a modern modular structure. Created dedicated `BattleVisualView` base and five specific subclasses (`EntityMarkerView`, `ProjectileView`, `BaseColumnView`, `FloatingTextView`, `ResultBannerView`) under a new `View/` folder. Implemented a generic and garbage-free `GameObjectPool<T>` with 100% robust dynamic `AddComponent` and procedural white-quad fallbacks to guarantee exact visual compatibility on raw unconfigured prefabs. Designed and integrated a deterministic visibility cap (limit 600) combined with a quantized distance bucket (size 200milli) that represents local entity swarms by deterministic numeric ID tiebreakers and prioritizes frontline rendering (SideA descending, SideB ascending progress coords) dynamically without visual flicker. Verified clean `ValidateClient.csproj` command-line compilation with 0 errors.
- **CLIENT_APP visual element prefab-ization (2026-05-31)**: Converted procedural rendering of Battle Entities, Projectiles, Bases, Floating Damage Text, and Result Banner in `StageBattleWorldView.cs` to use Unity `Resources.Load` prefab instantiations with robust procedural fallbacks if prefabs are missing or lack target component scripts. Defined canonical Resources path constants (`Battle/EntityMarker`, `Battle/Projectile`, `Battle/BaseColumn`, `Battle/FloatingDamageText`, `Battle/ResultBanner`) and verified clean `ValidateClient.csproj` command-line compilation. Logged non-spamming single-time warning alerts at load time upon detecting prefab/component absence. Staged and committed changes successfully under standard commit hook checks.
- **CLIENT_APP uGUI Battle HUD MVP (2026-05-30)**: Implemented the clean `uGUI Battle HUD` based on the MVP architecture (Presenter + View + Single Input Router). Created `StageBattlePresenter.cs` managing slot/lane selection, keyboard routing (1..4, Q/E, W/R/T/Space/F8/A/F6), and intent dispatching to StageBattleManager. Created `StageBattleHudView.cs` with inspector-serializable references (Text, Button, Slider) that maps ViewModel values into UI elements and binds buttons to Presenter intents. Demoted `StageAppDebugController`'s OnGUI to F9 togglable check and integrated Presenter selection state. Exposes slot definitions read-only from StageBattleSession. Created `HUD_PREFAB_GUIDE.md` outlining the Canvas tree hierarchy, inspector binding table, and manual verification instructions. Verified compile-clean (0 errors) on both `ValidateClient.csproj` and actual Unity runtime.
- **CLIENT_lead post-integration hotfixes (2026-05-24)**: After the v0.5+v0.7 worker mirror, the lead session applied direct hotfixes — (a) repaired `StageAppDebugController` BattleResult API (`.Winner`→`.WinnerSide`) and migrated all input from legacy `Input.GetKeyDown` to the active Input System (`Keyboard.current`); (b) removed `DebugBattleRunner.AutoCreateInDebugBuilds` RuntimeInitializeOnLoadMethod so Stage path is the sole auto-bootstrap (DebugBattleRunner still works when manually attached); (c) added `StageBattleSession.PrevalidateApplicable` mirroring core ApplyCommand invariants (cooldown/pilot-deployed/knockout/energy incl. tentative same-tick spawn cost) on both player and SideB-AI paths to stop a rejected command from faulting the whole session; (d) auto-log `LastFailureDump` on first fault; (e) added missing `System.Collections.Generic` using (worker's ValidateClient.csproj isolated build missed it); (f) redesigned debug input to a slot-select model (1..4 select slot, Q/E cycle lane across all 3 lanes, W spawn drone, R deploy pilot, T recall, F8 step); (g) lowered prototype sim rate to 5 TPS (`TickSeconds = 1f/5f`) for readable playtesting. Three-lane stage + projectile/event visuals confirmed working in Play Mode by user.
- **BattleSim.Core v0.5+v0.7 Client Integration**: Implemented a complete client-side mirror integration of `BattleSim.Core` v0.5+v0.7 in the Unity client (`FrontierBastion_client`). Synced compiled `BattleSim.Core.dll` plugin, mirrored constructor signatures in `TroopCardData.cs` and `DebugBattleScenarioFactory.cs` (preserving F1/F2/F4 smoke regressions), restructured stage to a three-lane geometry (lane_ground_1, lane_ground_2, lane_air) matching handoff §15 geometry, configured a symmetric v0.5 4-card deck specifying `AttackKind` and `ProjectileSpeed` values, completely rewrote `StageBattleWorldView.cs` into a tick-guarded event-driven visual renderer (using `RecentEvents` loop, visual entities, visual projectiles, floating damage popups, knockback interpolations, recall/dying lifecycles, and object pooling), and added `StageAppDebugController.cs` ground lane toggle with a real-time OnGUI event log. Verified compile-clean (0 errors).
- **BattleSim.Core v0.6 Knockback Applied**: Added `KnockbackDistanceMilli = 200` constant, implemented target knockback with clamping within $[0, \text{LaneLengthMilli}]$ using the existing `FindLaneDefinition` helper (which throws `InvalidOperationException` on invalid lanes), integrated melee hit knockback immediately after damage in `RunCombat()`, and projectile hit knockback immediately after damage on arrival in `RunProjectiles()`. Verified with 6 new comprehensive unit tests (melee hits, projectile hits, projectile misses, clamping, base damage exemption, and cooldown position hold), all passing successfully alongside all existing tests and smoke scenarios.
- **BattleSim.Core v0.5 Projectile Combat Implemented**: Replaced the immediate ranged combat model with a deterministic, 2D-world-coordinate-aware projectile simulation model. Added `ProjectileHitRadiusMilli` (100) and `ProjectileDefaultTtlTick` (200) constants. Spawns `RuntimeProjectile` at launch with mathematical lane projection. Runs flight tick updates, checks target alive/dead, verifies distance within hit radius upon arrival. Supported by 9 comprehensive unit tests (hits, travel times, dodge misses, cross-lane targeting, and state snapshotting).
...
- Current public API is intentionally minimal: `BattleCoreDefaults` exposes only Phase 1 defaults (`TickRate=16`, `TickMilliseconds=62.5`, `FixedPointScale=10000`).
...
0. **INFRA session — Step 1 (Cleanup).** Historical work-order retired. User decisions were captured above; the session executes the file-deletion + workers.yaml edits + .gitignore append only. No new code.
